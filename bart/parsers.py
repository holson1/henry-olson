...
def depart_parse(xml):
    ...
    for trip in trips:
        ...
        for leg in legs:
            ...
            head_station_color = head_station_info.get(head_station, "ddd")

            title_string = "%s line" % (head_station)
            display_string = "%s %s -&gt; %s %s" % (time_depart, leg_origin, time_arrive, leg_destination)
            field = {"title": title_string, "value": display_string, "short": short}

            attachment['fields'].append(field)
            attachment['color'] = head_station_color
